...
const validateWithDamper = (report: number[]) => {
  if (!validateReport(report)) return false;

  for (let i = 0; i < report.length; i++) {
    const modifiedReport = [...report.slice(0, i), ...report.slice(i + 1)]
    if (!validateReport(modifiedReport)) return false;
  }
  return true;
}
const safeReports = Reports.filter((report: number[]) => !validateWithDamper(report));
